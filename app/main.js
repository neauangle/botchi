...
*/
ipcMain.handle('password-window-result', (event, password) => {
    if (passwordWindow) {
...
async function createPasswordWindow() {
    ...
    passwordWindow = new electron_1.BrowserWindow({
        backgroundColor: "#fff",
        width: 1400,
        height: 800,
        show: false,
...
async function createMainWindow() {
    ...
    */
    ...
    mainWindow.isCallbackTicketOpen = (callbackTicket) => {
        return !!callbackTicketToDataCapsules[callbackTicket];
    };
    const getSentDataCapsules = (callbackTicket) => {
    ...
    ipcMain.handle('check-password', (event, password) => checkPassword(password));
    ...
    */
    ipcMain.handle('change-password', async (event, password, scriptModuleDatabase) => {
    ...
    ipcMain.handle('get-init-user-data', (event, password) => getUserDataInitially(password)); //also initialises backends
    ipcMain.handle('update-accepted-plugins', async (event, plugins) => {
        writeConfidentialData(plugins.map((plugin) => plugin.hash), PLUGIN_HASHES_PATH);
    });
    ...
    ipcMain.handle('get-backend-infos', (event, val) => {
        ...
        for (const backend of backends) {
            info.push({
                trackers: backend.module.call('getTrackersMap'),
                name: backend.name,
    ...
    for (const event of Object.keys(trackerModuleSpec.BACKEND_API.functions.create.return.emitter.events)) {
        for (const backend of backends) {
            ...
            backend.module.emitter.on(event, (data) => {
    ...
    ipcMain.handle('save-globals', (event, globals) => writeConfidentialData(globals, GLOBALS_PATH));
...
async function checkNetworkConnectivity(args) {
    const addresses = args.addresses;
    let msAtStart = Date.now();
    let completed = false;
    let result = false;
    for (const address of addresses) {
        dns.lookupService(address.ip, address.port, (error, hostname, service) => {
            if (!completed) {
                if (error) {
...
function showFileDialogue(options) {
...
function saveBotGroup(botGroup) {
    if (!fs.existsSync(BOT_GROUPS_FOLDER_PATH)) {
        fs.mkdirSync(BOT_GROUPS_FOLDER_PATH, { recursive: true });
    }
    let fileName;
...
function removeBotGroup(botGroup) {
    const filePath = path.join(BOT_GROUPS_FOLDER_PATH, botGroup.fileName);
    if (fs.existsSync(filePath)) {
        fs.unlinkSync(filePath);
    }
}
function saveBotGroupAs(botGroup, filePath) {
    delete botGroup.fileName;
    fs.writeFileSync(filePath, JSON.stringify(botGroup));
}
function loadBotGroup(filePath) {
    const botGroup = JSON.parse(fs.readFileSync(filePath, { 'encoding': 'utf-8' }));
    botGroup.fileName = path.basename(filePath);
    return botGroup;
}
function getLogFolderPath(botFullId, runNumberAsString) {
    const logFolderPath = path.join(LOGS_FOLDER_PATH, dateFolderName, sessionFolderName, botFullId, 'run_' + runNumberAsString);
    if (!fs.existsSync(logFolderPath)) {
        fs.mkdirSync(logFolderPath, { recursive: true });
    }
    return logFolderPath;
}
function logBotInstance(botInstance, botFullId, runNumberAsString) {
    fs.writeFileSync(path.join(getLogFolderPath(botFullId, runNumberAsString), 'botInstance.json'), JSON.stringify(botInstance, null, "   "), () => { });
}
function logPageTrace(botFullId, runNumberAsString, pageIndex, innerHTML) {
    fs.writeFileSync(path.join(getLogFolderPath(botFullId, runNumberAsString), pageIndex), innerHTML);
}
function getPageTrace(botFullId, runNumberAsString, pageIndex) {
...
function saveWorkspaceAs(botGroups, filePath) {
    for (const botGroup of botGroups) {
        delete botGroup.fileName;
    }
...
function getUserDataInitially(password) {
    if (password === '') {
...
function readConfidentialData(filePath, defaultValue) {
    try {
...
function writeConfidentialData(json, filePath) {
    if (!fs.existsSync(filePath)) {
        fs.mkdirSync(path.dirname(filePath), { recursive: true });
    }
